...
def morning_resolver(message):
    return resolve_regex_with_probability(ur'(?iu)(.{,6}прив.{0,20})|(.{,3}утро.{0,20})', 0.9, message.text)


def night_resolver(message):
    return resolve_regex_with_probability(ur'(?iu)(.{,6}ночи.{0,20}))', 0.9, message.text)
